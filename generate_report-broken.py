    ...
    try:
        with open(input_file, 'r', encoding='utf-8', errors='replace', newline='') as fin:
            ...
            for row in reader:
                ...
                # Check Booleans
                c1 = row.get('Class_1_Found', 'False').strip() == 'True'
                c2 = row.get('Class_2_Found', 'False').strip() == 'True'
                c3 = row.get('Class_3_Found', 'False').strip() == 'True'
                bc = row.get('Burned_Clay_Found', 'False').strip() == 'True'
                bc_only = row.get('Burned_Clay_Only', 'False').strip() == 'True'
                
                is_pre = row.get('Is_Prehistoric', 'False').strip() == 'True'
